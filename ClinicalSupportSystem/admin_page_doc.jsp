...
<body style="font-family: 'Aclonica', sans-serif; background: linear-gradient(to bottom right,white,#76CE9F );" >
    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-primary my-nv">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent" >
          <ul class="navbar-nav ml-auto" >
            <li class="nav-item">
              <a class="nav-link" href="admin_page.jsp">User</a>
            </li>
            <li class="nav-item">
    ...
    <div class="container-fluid" style="height:500px;background:linear-gradient(to bottom right,#84a9ac7c,#dcedc17c ),url(img/user_details_admin.jpg);background-repeat:no-repeat;background-size:cover;border-top:0.5px solid blue;">
        <div class="container" id="wctobynd">
          Welcome Admin<br>
        <small style="font-size:20px;">Check Your All Doctor's Details</small>
      </div>
    </div>
    

    <!--div for doctor details-->
    <div class="container-fluid card card-body" id="doc_details" style="margin-top:2px;">
    <h5 class="card-title">All Registred Doctor Details</h5>
    ...
    <%
         try{
            ...
            Statement s=null;
            ResultSet rs=null;
            String qry="select * from doc_info";
            ...
            while(rs.next())
            {
                %>
                    <tr>
                        <td><% out.print(rs.getString("LICENSE_KEY"));%></td>
                        <td><% out.print(rs.getString("NAME"));%></td>
                        <td><% out.print(rs.getString("MOBILE"));%></td>
                        <td><% out.print(rs.getString("EMAIL"));%></td>
                        <td><% out.print(rs.getString("GENDER"));%></td>
                        <td><% out.print(rs.getString("ADDRESS")+","+rs.getString("DISTRICT")+","+rs.getString("STATE"));%></td>
                        <td><% out.print(rs.getString("SPECIALIZATION"));%></td>
                        <td><% out.print(rs.getString("EB"));%></td>
                        <td><% out.print(rs.getString("APP_DAY"));%></td>
                        <td><% out.print(rs.getString("APP_TIME"));%></td>
                        <td><% out.print(rs.getString("STATUS"));%></td>
                        <td>
                          <form action="doc_verification.jsp" method="POST">
                                <input type="text"  style="display:none;" value="<%=rs.getString("LICENSE_KEY")%>" name="d_lice"/>
                                <button type="submit" class="btn btn-success btn-sm">Verified</button>
                          </form>
                        </td>
                        <td>
                            <form action="delete_doc.jsp" method="POST">
                                <input type="text" style="display:none;" value="<%=rs.getString("LICENSE_KEY")%>" name="d_lic"/>
                                 <input type="text" style="display:none;" value="<%=rs.getString("EMAIL")%>" name="d_mail"/>
